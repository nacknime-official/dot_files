...
app = '"Spotify"'
pactl = (
    subprocess.check_output(["pactl", "list", "sink-inputs"], env=env)
    .decode()
    .strip()
    .split()
)
if app in pactl:
    ...
    sink_id = sink_id[1:]
    volume = volume[:-1]
    if (int(volume) < 100 and operation == "+") or operation == "-":
        subprocess.run(["pactl", "set-sink-input-volume", sink_id, f"{operation}1%"])
